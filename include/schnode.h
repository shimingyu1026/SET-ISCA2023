...
public:
  ...
  virtual void print_scheme(std::string pad = "",
                            std::ostream &os = std::cout) const = 0;
  // Prints only the structure of the RA Tree.
  virtual void print_tree(std::string pad = "",
                          std::ostream &os = std::cout) const = 0;
...
public:
  ...
  // Generate chiplet simulation trace with deadlock-free transfer sequences
  void gen_chiplet_trace(std::ostream &os) const;
  virtual void
  add_workload_and_dfs(len_t batch_offset, len_t segment,
                       std::vector<Json::Value> &workload_list) const = 0;
...
private:
  ...
  CoreMapper::CoreMapping
      tileSch; // scheduling scheme of this layer (tiling, etc.)
...
public:
  ...
  virtual void
  add_workload_and_dfs(len_t batch_offset, len_t segment,
                       std::vector<Json::Value> &workload_list) const override;
