...
SchNode::sn_ptr SchNode::newNode(LTreeNode *_node, const Cluster &_c,
                                 Cut *parent) {
...
bool SchNode::is_valid() const { return valid; }

bool SchNode::is_DRAM_cut() const {
  return parent == nullptr && type == NodeType::T;
}

SchNode::NodeType SchNode::get_type() const { return type; }

const Cluster &SchNode::get_cluster() const { return cluster; }

SchNode::SchCost SchNode::get_cost() const { return cost; }

const NoC &SchNode::get_noc() const { return noc; }

const BufferUsage &SchNode::get_buf_usage() const { return buf_usage; }

const BufferUsage &SchNode::get_ifm_usage() const { return ifm_usage; }

const BufferUsage &SchNode::get_wgt_usage() const { return wgt_usage; }

energy_t SchNode::get_ubuf_energy() const { return ubuf_energy; }

energy_t SchNode::get_buf_energy() const { return buf_energy; }

energy_t SchNode::get_bus_energy() const { return bus_energy; }

energy_t SchNode::get_mac_energy() const { return mac_energy; }
...
LNode::LNode(LTreeNode *_node, const Cluster &_c, SchNode::cut_ptr _parent)
    : SchNode(NodeType::L, _c, _parent, _node->get_tot_batch()),
      layerid(_node->layers().first()),
      layert(network->getNode(
          layerid)), /*place_sch(cluster, layert, _node->get_tot_batch()),*/
...
void LNode::searchLayer() {
  ...
  if (!place_sch.getWgtL().update(layert.hasWgtPrevs() ? ifm_usage
                                                       : wgt_usage)) {
    valid = false;
    return;
  }
...
bool LNode::contains(lid_t _layerid) const { return _layerid == layerid; }

const Node &LNode::getLayer() const { return layert; }

const PlaceSch &LNode::get_place_sch() const { return place_sch; }

const Bitset &LNode::get_dirp_set() const { return dirp_set; }

bool LNode::get_to_dram() const { return to_dram; }
...
void LNode::print_scheme(std::string pad, std::ostream &os) const {
  ...
  os << ' ' << cost << " Ubuf/Buf/Bus/Mac:" << ubuf_energy << '/' << buf_energy
     << '/' << bus_energy << '/' << mac_energy;
...
struct LayerExecInfo {
  ...
  std::vector<std::string>
      chiplet_list;     // List of chiplets that execute this layer
  cidx_t chiplet_index; // Index of current chiplet in the list (0-based)

  LayerExecInfo(const std::string &name,
...
// For each layer, we add it to the schedule of chiplets that actually execute
// it
static void collect_chiplet_schedule(
    const SchNode *node,
  ...
  for (const auto &group : grouped) {
    ...
    for (const auto &t : group.second) {
      os << "    Transfer: Chiplet(" << static_cast<int>(t.from_chiplet.x)
         << "," << static_cast<int>(t.from_chiplet.y) << ") -> ";
      if (t.transfer_type == "unicast") {
        os << "Chiplet(" << static_cast<int>(t.to_chiplets[0].x) << ","
           << static_cast<int>(t.to_chiplets[0].y) << ")";
      } else {
        os << "[";
        for (size_t i = 0; i < t.to_chiplets.size(); ++i) {
          if (i > 0)
            os << ", ";
          os << "(" << static_cast<int>(t.to_chiplets[i].x) << ","
             << static_cast<int>(t.to_chiplets[i].y) << ")";
        }
        os << "]";
      }
      os << " | Type: " << t.transfer_type << " | Data size: " << t.data_size
         << " | Range: C[" << t.data_range.c.from << ":" << t.data_range.c.to
         << "] B[" << t.data_range.b.from << ":" << t.data_range.b.to << "] H["
         << t.data_range.h.from << ":" << t.data_range.h.to << "] W["
         << t.data_range.w.from << ":" << t.data_range.w.to << "]" << std::endl;
    }
  }
  os << std::endl;
}
...
Cut::Cut(SchNode::NodeType t, LTreeNode *node, const Cluster &_c,
         SchNode::cut_ptr _parent)
...
void Cut::add(SchNode *child) { children.push_back(child); }
...
bool Cut::contains(lid_t layerid) const { return layers.contains(layerid); }

const SchNode::sn_vec &Cut::getChildren() const { return children; }

len_t Cut::get_num_bgrp() const { return num_bgrp; }
...
void TCut::construct(LTreeNode *node) {
      ...
      if (last_p == nullptr) {
        ...
        if (wgt_shift) {
          ...
          buf_usage.max_with(p->get_ifm_usage() + last_p->get_buf_usage() +
                             last_p->get_wgt_usage() + p->get_wgt_usage());
        } else {
          buf_usage.max_with(p->get_ifm_usage() + last_p->get_buf_usage());
        }
        if (!buf_usage) {
  ...
  if (!is_top) {
    if (num_bgrp == 1) {
      ...
      if (wgt_shift) {
        // With weight shift, weight is handled just like ifmap.
        buf_usage.max_with(children.front()->get_ifm_usage() +
                           children.front()->get_wgt_usage() +
                           last_p->get_buf_usage() + last_p->get_wgt_usage());
      } else {
        buf_usage.max_with(children.front()->get_ifm_usage() +
                           last_p->get_buf_usage());
      }
    }
...
SCut::SCut(LTreeNode *_node, const Cluster &_c, SchNode::cut_ptr _parent)
    : Cut(NodeType::S, _node, _c, _parent), stage(_node->get_stages()),
      num_stage(_node->get_num_stage()) {
...
bool SchNode::SchCost::isValid() const { return energy < energy_inf; }
...
std::ostream &operator<<(std::ostream &os, const SchNode::SchCost &cost) {
  return os << "E:" << cost.energy << ", T:" << cost.time
            << ", Cost:" << cost.cost();
}
...
std::vector<bool> SchNode::from_core, SchNode::weight_from_core,
    SchNode::to_dram;
...
Json::Value SchNode::IR_gen() const {
    ...
    for (Json::Value &wl : workload_list[i]) {
      for (Json::Value &buffer : wl["buffer"]) {
        if (buffer["type"] == "ifmap") {
          buffer["workload_id"] =
              workload_list[i][wlid[i][name_to_id[buffer["layer"].asString()]]
                                   [buffer["lower"][0u].asUInt()]]
                           ["workload_id"];
          buffer["source"] = workload_list
              [i][wlid[i][name_to_id[buffer["layer"].asString()]]
                      [buffer["lower"][(Json::Value::UInt)0].asUInt()]]
              ["ifmap_temp"]
              [buffer["layer"].asString() + "_" +
               std::to_string(buffer["lower"][(Json::Value::UInt)0].asUInt())]
              ["source"];
      ...
      if (wl.isMember("ifmap")) {
        if (!from_core[wl["workload_id"].asUInt()]) {
          ...
          buffer["block"] = ((wl["ifmap"]["upper"][0u].asUInt() -
                              wl["ifmap"]["lower"][0u].asUInt() + 1) *
                                 (wl["ifmap"]["upper"][1].asUInt() -
                                  wl["ifmap"]["lower"][1].asUInt() + 1) *
                                 (wl["ifmap"]["upper"][2].asUInt() -
                                  wl["ifmap"]["lower"][2].asUInt() + 1) *
                                 (wl["ifmap"]["upper"][3].asUInt() -
                                  wl["ifmap"]["lower"][3].asUInt() + 1) +
                             1023) >>
                            10;
          buffer["source"] =
              wl["ifmap_temp"][buffer["layer"].asString() + "_" +
                               std::to_string(buffer["lower"][0u].asUInt())]
                ["source"];
          for (Json::Value &source : buffer["source"]) {
            buffer["transfer_id"].append(source["transfer_id"]);
          }
          ...
          if (last_wl && (*last_wl)["workload_id"] >=
                             wl["ifmap"]["max_workload_id"].asUInt()) {
      ...
      if (wl.isMember("weight") && wl["weight"].isMember("from_ofmap")) {
        if (!weight_from_core[wl["workload_id"].asUInt()]) {
          ...
          buffer["source"] =
              wl["weight_temp"][buffer["layer"].asString() + "_" +
                                std::to_string(buffer["lower"][0u].asUInt())]
                ["source"];
          for (Json::Value &source : buffer["source"]) {
            buffer["transfer_id"].append(source["transfer_id"]);
          ...
          wl["buffer"].append(buffer);
          if (last_wl && (*last_wl)["workload_id"] >=
                             wl["weight"]["max_workload_id"].asUInt()) {
            (*last_wl)["buffer"].append(buffer);
          }
        }
      }
  ...
  ret["top_batch_cut"] = root->type != SchNode::NodeType::L
                             ? dynamic_cast<const Cut *>(root)->get_num_bgrp()
                             : 1;
  ret["-1"] = DRAM;
...
void LNode::add_workload_and_dfs(
    len_t batch_offset, len_t segment,
    std::vector<Json::Value> &workload_list) const {
  ...
  for (auto part : ofm_parts) {
    ...
    if (REF_IS_INSTANCE(layert.layer(), ConvLayer) && !layert.hasWgtPrevs()) {
      ...
      if (DRAM_weight_pos.count(key)) {
        ...
        if (root->get_type() != NodeType::L) {
          batch_size =
              tot_batch / dynamic_cast<const Cut *>(root)->get_num_bgrp();
        } else {
        ...
        ConvLayer::Workload wl =
            static_cast<const ConvLayer &>(layert.layer()).get_workload();
    ...
    FOR_BITSET(layerno, prev) {
      ...
      assert(layert.getIfmPrevs().contains(layerno) ^
             layert.getWgtPrevs().contains(layerno));
      const auto input_range =
          layert.getIfmPrevs().contains(layerno) ? ofmap_range : weight_range;
      const auto real_prev_channel_offset =
          layert.getIfmPrevs().contains(layerno) ? prev_channel_offset : 0;
      if (dirp_set.contains(layerno)) {
        for (auto prev_part : lnode->get_place_sch().getOfmL()) {
          for (len_t prev_batch_offset = 0; prev_batch_offset < tot_batch;
               prev_batch_offset += lnode->num_batch) {
            ...
            ifmap["channel"].append(real_prev_channel_offset +
                                    intersect.c.from);
            ifmap["channel"].append(real_prev_channel_offset + intersect.c.to -
                                    1);

            /*vol_t ifmap_size = 1;
            for(int i=0; i<4; ++i){
                    ifmap_size *= ifmap["source"]["upper"][i].asUInt() -
            ifmap["source"]["lower"][i].asUInt();
            }
            ...
            ifmap["id"] = from_id;
            // ifmap["workload_id"] =
            // workload_list[from_id][wlid[from_id][layerno][intersect.b.from]]["workload_id"];
            ifmap["layer_name"] = node.name();

            jsonindex_t prev_wlid = wlid[from_id][layerno][intersect.b.from];
            wlid_t prev_workload_id =
                workload_list[from_id][prev_wlid]["workload_id"].asUInt();
            if (layert.getIfmPrevs().contains(layerno)) {
              max_from_workload_id =
                  std::max(max_from_workload_id, prev_workload_id);
            } else {
              weight_max_from_workload_id =
                  std::max(weight_max_from_workload_id, prev_workload_id);
            }
            if (ofmapid[prev_workload_id].count(intersect)) {
              ifmap["transfer_id"] =
                  workload_list[from_id][prev_wlid]["ofmap"]
                               [ofmapid[prev_workload_id][intersect]]
                               ["transfer_id"];
            } else {
            ...
            if (layert.getIfmPrevs().contains(layerno)) {
              ...
              workload["ifmap_temp"][layert.name() + "_" +
                                     std::to_string(range.b.from)]["source"]
                  .append(ifmap);
            } else {
              workload["weight"]["transfer_id"].append(ifmap["transfer_id"]);
              workload["weight_temp"][layert.name() + "_" +
                                      std::to_string(range.b.from)]["source"]
                  .append(ifmap);
            }
            ...
            if (ofmapid[prev_workload_id].count(intersect)) {
              workload_list[from_id][prev_wlid]["ofmap"]
                           [ofmapid[prev_workload_id][intersect]]["destination"]
                               .append(destination);
            } else {
              Json::Value ofmap;
              ...
              ofmapid[prev_workload_id][intersect] =
                  workload_list[from_id][prev_wlid]["ofmap"].size();
        ...
        int lower_c, upper_c;
        lower_c = std::max(0, (int)input_range.c.from -
                                  (int)real_prev_channel_offset);
        upper_c =
            std::min((int)node.layer().ofmap_shape().c,
                     (int)input_range.c.to - (int)real_prev_channel_offset);
        if (lower_c < upper_c) {
          ...
          for (int i = 0; i < 4; ++i) {
            ifmap_size *=
                ifmap["upper"][i].asUInt() - ifmap["lower"][i].asUInt() + 1;
          }
          ...
          key["type"] =
              layert.getIfmPrevs().contains(layerno) ? "ifmap" : "weight";

          tfid_t ofmap_transfer_id;

          if (DRAM_ofmap_pos.count(key)) {
            ofmap_transfer_id =
                DRAM["out"][DRAM_ofmap_pos[key]]["transfer_id"].asUInt();
          } else {
          ...
          for (auto prev_part : lnode->get_place_sch().getOfmL()) {
            for (len_t prev_batch_offset = 0; prev_batch_offset < tot_batch;
              ...
              if (ofmapid[prev_workload_id].count(prev_range)) {
                transfer_id =
                    workload_list[from_id][prev_wlid]["ofmap"]
                                 [ofmapid[prev_workload_id][prev_range]]
                                 ["transfer_id"]
                                     .asUInt();
                ...
                SchNode::to_dram[workload_list[from_id][prev_wlid]
                                              ["workload_id"]
                                                  .asUInt()] = true;
                ofmapid[prev_workload_id][prev_range] =
                    workload_list[from_id][prev_wlid]["ofmap"].size();
              ...
              if (!DRAM["in"][DRAM_ifmap_pos[transfer_id]]["related_ofmap_map"]
                       .isMember(std::to_string(ofmap_transfer_id))) {
                DRAM["in"][DRAM_ifmap_pos[transfer_id]]["related_ofmap"].append(
                    ofmap_transfer_id);
                DRAM["in"][DRAM_ifmap_pos[transfer_id]]["related_ofmap_map"]
                    [std::to_string(ofmap_transfer_id)] = true;
              }
      ...
      if (layert.getIfmPrevs().contains(layerno)) {
        prev_channel_offset +=
            network->getNode(layerno).layer().ofmap_shape().c;
      }
    ...
    if (prev.count() == 0) {
      ...
      workload["ifmap_temp"][layert.name() + "_" + std::to_string(range.b.from)]
              ["source"]
                  .append(ifmap);
      Json::Value destination;
      destination["id"] = core_id;
    ...
    FOR_BITSET(layerno, next) {
      ...
      if (lnode->layert.getIfmPrevs().contains(layerid)) {
        FOR_BITSET(lid, prev_set) {
          if (lid != layerid) {
            next_channel_offset +=
                network->getNode(lid).layer().ofmap_shape().c;
          } else {
            break;
          }
        }
      ...
      if (lnode->get_dirp_set().contains(layerid)) {
        for (auto next_part : lnode->get_place_sch().getOfmL()) {
          for (len_t next_batch_offset = 0; next_batch_offset < tot_batch;
               next_batch_offset += lnode->num_batch) {
            ...
            Json::Value buffer;
            buffer["type"] = lnode->getLayer().getIfmPrevs().contains(layerid)
                                 ? "ifmap"
                                 : "weight";
            if (!lnode->getLayer().getIfmPrevs().contains(layerid))
    ...
    if (REF_IS_INSTANCE(layert.layer(), ConvLayer) && !layert.hasWgtPrevs()) {
      ...
      if (batch_offset % batch_size == 0) {
        ...
        if (workload_list[core_id].size() &&
            get_lca(this,
                    root->get_lnode_by_id(
                        name_to_id[workload_list[core_id]
                                                [workload_list[core_id].size() -
                                                 1]["layer_name"]
                                                    .asString()])) != root) {
          if (workload_list[core_id][workload_list[core_id].size() - 1]
                           ["layer_name"] != layert.name()) {
            workload_list[core_id][workload_list[core_id].size() - 1]["buffer"]
                .append(weight);
          }
        }
      }
...
void TCut::add_workload_and_dfs(len_t batch_offset, len_t segment,
                                std::vector<Json::Value> &workload_list) const {
...
void SCut::add_workload_and_dfs(len_t batch_offset, len_t segment,
                                std::vector<Json::Value> &workload_list) const {
