...
#include <yaml-cpp/yaml.h> // yaml-cpp for YAML output
...
static void init_core(const std::string &core_type, Core *&core,
                      CoreMapper *&cMapper);
...
int main(int argc, char **argv) {
    ...
    if (!config_file.empty()) {
      ...
      while (true) {
        ...
        if (config_name == "exp") {
          ...
          throw std::invalid_argument("Config name \"" + config_name +
                                      "\" not recognized!");
        }
  ...
  // Print layer information (shape and dependencies) in YAML format
  auto print_layer_info = [&](const std::string &exp_name) {
    YAML::Emitter yaml;
    yaml << YAML::BeginMap;

    // Header information
    yaml << YAML::Key << "layer_information";
    yaml << YAML::Value << YAML::BeginMap;
    yaml << YAML::Key << "title" << YAML::Value
         << "Layer Information: Shape and Dependencies";
    yaml << YAML::Key << "note" << YAML::Value
         << "In this framework, many operations are represented as ConvLayer:\n"
         << "  - Matrix multiplications (GEMM) are represented as 1x1 "
            "convolutions (R=1, S=1)\n"
         << "  - Fully connected layers inherit from ConvLayer\n"
         << "  - Transformer attention projections (Q, K, V) are represented "
            "as ConvLayer\n"
         << "  - Feedforward layers in transformers are represented as "
            "ConvLayer\n"
         << "This is because 1x1 convolution is mathematically equivalent to "
    ...
    for (lid_t i = 0; i < num_layer; ++i) {
      ...
      yaml << YAML::BeginMap;
      yaml << YAML::Key << "layer_id" << YAML::Value << i;
      yaml << YAML::Key << "name" << YAML::Value << node.name();
      ...
      if (REF_IS_INSTANCE(layer, GroupConvLayer)) {
        const GroupConvLayer *gconv =
            static_cast<const GroupConvLayer *>(&layer);
        const auto &wl = gconv->get_workload();
        layer_type = "GroupConvLayer";
        yaml << layer_type;
        has_workload = true;
        std::ostringstream wl_str;
        wl_str << "G=" << wl.G << ", C=" << wl.C << ", K=" << wl.K 
               << ", R=" << wl.R << ", S=" << wl.S << ", H=" << wl.H << ", W=" << wl.W;
        yaml << YAML::Key << "workload" << YAML::Value << wl_str.str();
        ...
        has_workload = true;
        std::ostringstream wl_str;
        wl_str << "C=" << wl.C << ", K=" << wl.K << ", R=" << wl.R << ", S=" << wl.S 
               << " (R=IH, S=IW for FC layer)";
        yaml << YAML::Key << "workload" << YAML::Value << wl_str.str();
        ...
        yaml << layer_type;
        if (is_gemm) {
          yaml << YAML::Key << "is_gemm" << YAML::Value << true;
        }
        has_workload = true;
        std::ostringstream wl_str;
        wl_str << "C=" << wl.C << ", K=" << wl.K << ", R=" << wl.R << ", S=" << wl.S 
               << ", H=" << wl.H << ", W=" << wl.W << ", sH=" << wl.sH << ", sW=" << wl.sW;
        yaml << YAML::Key << "workload" << YAML::Value << wl_str.str();
      } else if (REF_IS_INSTANCE(layer, PoolingLayer)) {
        const LRLayer *lr = static_cast<const LRLayer *>(&layer);
        const auto &wl = lr->get_workload();
        layer_type = "PoolingLayer";
        yaml << layer_type;
        has_workload = true;
        std::ostringstream wl_str;
        wl_str << "K=" << wl.K << ", H=" << wl.H << ", W=" << wl.W 
               << ", R=" << wl.R << ", S=" << wl.S << ", sH=" << wl.sH << ", sW=" << wl.sW;
        yaml << YAML::Key << "workload" << YAML::Value << wl_str.str();
      } else if (REF_IS_INSTANCE(layer, EltwiseLayer)) {
        layer_type = "EltwiseLayer";
        yaml << layer_type;
        ...
        std::ostringstream wl_str;
        wl_str << "K=" << wl.K << ", H=" << wl.H << ", W=" << wl.W 
               << ", N=" << wl.N << ", R=" << wl.R << ", S=" << wl.S 
               << ", sK=" << wl.sK << ", sH=" << wl.sH << ", sW=" << wl.sW;
        yaml << YAML::Key << "workload" << YAML::Value << wl_str.str();
      } else {
        layer_type = "Unknown";
        yaml << layer_type;
      ...
      // Shapes - use inline format (C, H, W)
      const fmap_shape &ifmap = layer.tot_ifmap_shape();
      const fmap_shape &ofmap = layer.ofmap_shape();

      std::ostringstream ifmap_str;
      ifmap_str << "(" << ifmap.c << ", " << ifmap.h << ", " << ifmap.w << ")";
      yaml << YAML::Key << "input_shape" << YAML::Value << ifmap_str.str();

      std::ostringstream ofmap_str;
      ofmap_str << "(" << ofmap.c << ", " << ofmap.h << ", " << ofmap.w << ")";
      yaml << YAML::Key << "output_shape" << YAML::Value << ofmap_str.str();

      if (layer.weight_size() > 0) {
        const fmap_shape &wgt = layer.weight_shape();
        std::ostringstream wgt_str;
        wgt_str << "(" << wgt.c << ", " << wgt.h << ", " << wgt.w << ", size: " << layer.weight_size() << ")";
        yaml << YAML::Key << "weight_shape" << YAML::Value << wgt_str.str();
      } else {
        yaml << YAML::Key << "weight" << YAML::Value << YAML::Null;
      }

      // Dependencies
      const Bitset &prevs = node.getPrevs();
      if (prevs.count() > 0) {
        yaml << YAML::Key << "dependencies" << YAML::Value << YAML::BeginSeq;
        FOR_BITSET(it, prevs) { yaml << it; }
        yaml << YAML::EndSeq;
      } else {
        yaml << YAML::Key << "dependencies" << YAML::Value << YAML::Null;
        yaml << YAML::Key << "is_input_layer" << YAML::Value << true;
      }

      // Next layers
      const Bitset &nexts = node.get_nexts();
      if (nexts.count() > 0) {
        yaml << YAML::Key << "next_layers" << YAML::Value << YAML::BeginSeq;
        FOR_BITSET(it, nexts) { yaml << it; }
        yaml << YAML::EndSeq;
      } else {
        yaml << YAML::Key << "next_layers" << YAML::Value << YAML::Null;
      }
    ...
    // Write to file
    std::ofstream out(exp_name + "layer_info.yaml");
    out << yaml.c_str();
  };
  ...
  std::cout << " Mesh " << static_cast<int>(Cluster::xlen) << '*'
            << static_cast<int>(Cluster::ylen);
  ...
  for (len_t lb = tot_batch; lb > 0; lb /= 2) {
    init_tree =
        new LTreeNode(Bitset(), tot_batch, nullptr, LTreeNode::NodeType::T);
    for (lid_t i = 0; i < num_layer; ++i) {
  ...
  auto search = [&](const char *method, bool has_S, bool has_T) {
    ...
    for (int i = 0; i < tries; ++i) {
      ...
      thr[i] = new std::thread(&SAEngine::SA_search, searchEngine[i],
                               std::ref(try_sch[i]), std::ref(c), 2, SA_type);
    }
    for (int i = 0; i < tries; ++i) {
      thr[i]->join();
      delete thr[i];
  ...
  auto our_search = [&](const char *method,
                        const WholeSch &init_sch) -> WholeSch {
    if (!init_sch) {
    ...
    for (int i = 0; i < tries; ++i) {
      try_sch[i] = init_sch.copy();
      thr[i] = new std::thread(&SAEngine::SA_search, searchEngine[i],
                               std::ref(try_sch[i]), std::ref(c), 0, 0);
    }
    for (int i = 0; i < tries; ++i) {
      thr[i]->join();
      delete thr[i];
...
static void init_core(const std::string &core_type, Core *&core,
  ...
  if (core_type == "polar") {
    ...
    EyerissCore *e_core =
        new EyerissCore(s2, LR_mac_num, LR_mac_cost, eBus, eBuf);
    core = e_core;
    cMapper = new EyerissMapper(*e_core);
  } else {
    throw std::invalid_argument("Core type \"" + core_type +
                                "\" not recognized!");
  }
}
